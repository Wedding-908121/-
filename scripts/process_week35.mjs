// Week 35 processor - fetch manual URLs + auto sources + AI summarize
...
function makeId(url, title, salt) {
  return 'w35-' + createHash('md5').update(url + '|' + title + '|' + salt).digest('hex').substring(0, 12);
}
...
async function fetchManualUrls() {
  ...
  for (let i = 0; i < urlEntries.length; i++) {
    ...
    try {
      ...
      let pubDate = new Date('2026-08-24T00:00:00+08:00');
  ...
  try {
    ...
    for (const q of queries) {
      ...
      for (const m of items) {
        ...
        const weekStart = new Date('2026-08-24T00:00:00+08:00');
        const weekEnd = new Date('2026-08-30T23:59:59+08:00');
  ...
  for (const [cat, query] of Object.entries(PAPER_QUERIES)) {
    ...
    try {
      const url = 'https://api.openalex.org/works?search=' + encodeURIComponent(query) +
        '&per_page=5&sort=publication_date:desc&filter=from_publication_date:2026-08-15' +
