...
// Version bump in app.js for data fetches
const appPath = new URL("app.js", outputDirectory);
let app = await readFile(appPath, "utf8");
app = app.replace(/articles\.json\?v=[^"'\s]*/g, "articles.json?v=" + version);
app = app.replace(/index\.json\?v=[^"'\s]*/g, "index.json?v=" + version);
await writeFile(appPath, app, "utf8");

// runtime-config
const endpoint = String(process.env.FEEDBACK_API_URL || "").replace(/\/+$/, "");
await writeFile(
  new URL("runtime-config.js", outputDirectory),
  "window.MECH_INTEL_CONFIG = Object.freeze(" + JSON.stringify({ feedbackEndpoint: endpoint }, null, 2) + ");\n",
  "utf8"
);
